...
def run_mpe(save_file, actor_learning_rate, critic_learning_rate, run_file='simple_search_team',
            n_game=6000, load_file=None):
    env = mpe_env(run_file, seed=SEED)
    convert_f = convert(obs_dim)
    convert_f.lstm_model.load_lstm(load_file)
    ...
    maddpg_agents.load_models(load_file, 6000)
    # 经验池初始化
    all_agent_exp = []
    ...
    for i_episode in range(n_game):
        ...
        for t in range(100):
            env.mpe_env.render()
            # 探索的幅度随机训练的进行逐渐减小
            if i_episode >= 3000 and ((i_episode - 3000) % 500 == 0):
            ...
            con_obs_n = new_con_obs_n
        maddpg_agents.logs(score_one_episode, 100 * (i_episode + 1))
...
if __name__ == '__main__':
    run_mpe('run_random_object', 1e-3, 1e-3, n_game=6000, run_file='simple_search_team', load_file='run4')
    # run_mpe('run_random_object_2', 1e-3, 1e-3, n_game=6000, run_file='simple_search_team', load_file='run4')
