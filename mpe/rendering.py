...
class Viewer(object):
    def __init__(self, width, height, display=None):
        ...
        self.traj = []
        # 设置长宽
        self.window = pyglet.window.Window(width=width, height=height, display=display)  # pyglet语句，产生窗口
        self.window.on_close = self.window_closed_by_user  # 关闭窗口的实现
        self.geoms = []
        self.onetime_geoms = []
...
class FilledPolygon(Geom):  # 以某种颜色填充多边形，这种颜色是在self._color定义的
    def __init__(self, v):
        Geom.__init__(self)
        self.v = v
...
def make_polyline(v, width=1):
    return PolyLine(v, False, width=width)
...
class PolyLine(Geom):
    def __init__(self, v, close, width=1):
        Geom.__init__(self)
        self.v = v
        self.close = close
