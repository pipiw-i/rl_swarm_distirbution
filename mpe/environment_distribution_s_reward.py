...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
                 ...
                 done_callback=None, post_step_callback=None, benchmark_callback=None,
                 get_attack_number_callback=None, shared_viewer=True, discrete_action=True):
    ...
    def step(self, action_n):
        ...
        for i, agent in enumerate(self.agents):
            ...
            info = {'individual_reward': reward_n[i][0]}
            env_info = self._get_info(agent)
            if 'fail' in env_info.keys():
                info['fail'] = env_info['fail']
            info_n.append(info)
        attack_number, attack_index = self.get_attack_number_callback(self.world)
        r_n = []
        for rew in reward_n:
            r_n.append(rew[0])
        ...
        self.current_step += 1
        return obs_n, r_n, done_n, info_n, attack_number, attack_index
    ...
    def _get_benchmark_pos(self):
        if self.benchmark_callback is None:
            return np.zeros(0)
        return self.benchmark_callback(self.world)
    ...
    def render(self, mode='human', close=False):
        ...
        for i in range(len(self.viewers)):
            ...
            if self.viewers[i] is None:
                ...
                from . import rendering  # 加载rendering文件
        ...
        for i in range(len(self.viewers)):
            ...
            for e, entity in enumerate(self.world.entities):
                ...
                if 'agent' in entity.name:
                    self.render_geoms[3 * e + 2].set_color(*entity.color, alpha=0.5)  # 设置颜色
                    if not entity.silent:
                    ...
                    self.render_geoms[2 * agent_number + e + 2].set_color(*entity.color)
                    if entity.channel is not None:
                        for ci in range(self.world.dim_c):
